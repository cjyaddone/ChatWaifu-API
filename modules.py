...
class ConvReluNorm(nn.Module):
  def __init__(self, in_channels, hidden_channels, out_channels, kernel_size, n_layers, p_dropout):
    ...
    self.conv_layers.append(nn.Conv1d(in_channels, hidden_channels, kernel_size, padding=kernel_size//2).to(device))
    self.norm_layers.append(LayerNorm(hidden_channels).to(device))
    self.relu_drop = nn.Sequential(
        nn.ReLU().to(device),
        nn.Dropout(p_dropout).to(device))
    for _ in range(n_layers-1):
      self.conv_layers.append(nn.Conv1d(hidden_channels, hidden_channels, kernel_size, padding=kernel_size//2).to(device))
      self.norm_layers.append(LayerNorm(hidden_channels).to(device))
...
class DDSConv(nn.Module):
    ...
    def __init__(self, channels, kernel_size, n_layers, p_dropout=0.):
        ...
        for i in range(n_layers):
            ...
            self.convs_sep.append(nn.Conv1d(channels, channels, kernel_size, groups=channels, dilation=dilation, padding=padding).to(device))
...
class ResidualCouplingLayer(nn.Module):
  def __init__(self,
    ...
    self.enc = WN(hidden_channels, kernel_size, dilation_rate, n_layers, p_dropout=p_dropout, gin_channels=gin_channels).to(device)
